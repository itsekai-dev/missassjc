...
function parseHtmlTable(html){ const $=cheerioLoad(html); const items=[]; $('table.resultados tbody tr').each((_,el)=>{ const t=$(el).find('td'); if(t.length>=4){ const nome=$(t[0]).find('a').first().text().trim().replace(/\s+/g,' '); const obsHtml=$(t[0]).find('span.obs').first().html()||''; const addressFragment=obsHtml.split('<br')[0]||''; const address=cheerioLoad(`<div>${addressFragment}</div>`)('div').text().trim().replace(/\s+/g,' '); const mapsUrl=address?`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`:''; const bairro=$(t[1]).text().trim().replace(/\s+/g,' '); const cidade=$(t[2]).text().trim().replace(/\s+/g,' '); const inicio=$(t[3]).text().trim(); items.push({nome,bairro,cidade,inicio,address,mapsUrl}) } }); return items }
